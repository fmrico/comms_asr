...
#include "comms_asr/ServiceServer.hpp"
#include "comms_asr/ServiceClient.hpp"
#include "comms_asr/ActionServer.hpp"
#include "comms_asr/ActionClient.hpp"
...
TEST(test_comms, service_server)
{
  ...
  auto server_node = std::make_shared<comms_asr::ServiceServer>();
...
TEST(test_comms, service_client)
{
  rclcpp::executors::SingleThreadedExecutor exe;
  auto server_node = std::make_shared<comms_asr::ServiceServer>();
  auto server_client = std::make_shared<comms_asr::ServiceClient>();
  exe.add_node(server_node);
...
{
  rclcpp::executors::SingleThreadedExecutor exe;
  auto server_node = std::make_shared<comms_asr::ActionServer>();
  ...
  auto action_client = rclcpp_action::create_client<comms_asr::ActionServer::NavigateToPose>(
    client_node.get(), "navigate_to_pose");

  ASSERT_TRUE(action_client->wait_for_action_server());
  auto goal_msg = comms_asr::ActionServer::NavigateToPose::Goal();

  auto send_goal_options =
    rclcpp_action::Client<comms_asr::ActionServer::NavigateToPose>::SendGoalOptions();
...
TEST(test_comms, action_client)
{
  rclcpp::executors::SingleThreadedExecutor exe;
  auto server_node = std::make_shared<comms_asr::ActionServer>();
  auto client_node = std::make_shared<comms_asr::ActionClient>();
  exe.add_node(server_node);
  exe.add_node(client_node);
